...
export default async function PerformancePage() {
  const checks = await prisma.performanceCheck.findMany({
  ...
  return (
    <>
      <PageHeader
        ...
        description="Historique des controles HTTP simples : statut, temps de reponse et poids declare."
      />
          ...
          <tbody className="divide-y divide-zinc-100">
            {checks.map((check) => (
              <tr key={check.id} className="hover:bg-zinc-50">
                ...
                <td className="px-4 py-3 text-zinc-600">{check.url}</td>
                <td className="px-4 py-3 text-zinc-600">{check.responseTimeMs ?? "n/a"} ms</td>
                <td className="px-4 py-3 text-zinc-600">{check.httpStatusCode ?? "n/a"}</td>
